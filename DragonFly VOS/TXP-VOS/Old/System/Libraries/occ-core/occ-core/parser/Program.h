#ifndef guard_occ_core_parser_Program_h
#define guard_occ_core_parser_Program_h
...
/**
    Implements a flat stream of characters constituting translation unit.
...
public:
    ...
    /** Backs the recently read char.
        @warning Do not unget an EOF, since reading EOF does not advance 
	         the stream position
      */
    void Unget() { --index; }
    ...
    /** Returns current character in the stream and advances the stream
        to the next character (provided EOF has not been reached) */
    virtual char Get();
...
#endif /* ! guard_occ_core_parser_Program_h */
