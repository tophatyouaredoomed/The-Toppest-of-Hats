...
#include <string.h>
#include <string>
// #include <strstream>  <- Why should we use deprecated stuff?
#include <sstream>
#include <sys/time.h>
...
#include <occ-core/parser/NonLeaf.h> // :TODO: cyclic dependency!!! get rid of it!
...
char* Ptree::ToString()
{
    ...
    else /*Removed deprecated code*/
    {
        std::ostringstream buf;
  	PrintOn(buf);
	buf << std::ends;
	char* result = new (GC) char[buf.str().length() + 1];
	std::strcpy(result, buf.str().c_str());
	return result;
    }
    /* Old quellcode
    else{
...
{
    assert(IsLeaf());
    return Lex::Reify(GetPosition(), GetLength(), value);
}

bool Ptree::Reify(char*& str)
{
    assert(IsLeaf());
    return Lex::Reify(GetPosition(), GetLength(), str);
}
