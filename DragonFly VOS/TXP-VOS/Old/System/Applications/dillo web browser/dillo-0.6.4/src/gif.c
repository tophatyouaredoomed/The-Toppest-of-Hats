 ...
 */
static void Gif_write(DilloGif *gif, void *Buf, gint BufSize);
...
{
   guchar *buf;
   ssize_t bufsize, bytes_consumed;
...
#ifdef VERBOSE
   g_print("exit Gif_write, bufsize=%d\n", bufsize);
#endif
}
 ...
 */
static size_t Gif_do_extension(DilloGif *gif, guchar Label,
                               const guchar *buf,
                               size_t BSize)
{
   switch (Label) {
 ...
 * ?
 */
static gint Gif_check_sig(DilloGif *gif, const guchar *ibuf, size_t Size)
{
...
#ifdef VERBOSE
   g_print("Gif_process_bytes: initial state %d, final state %d,"
           "%d bytes consumed\n",
